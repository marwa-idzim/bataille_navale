...
int main(void) {
	...
	/* construction de la structure jeu & nommage du joueur */
	j = jeu_construire();
	printf("jeu_est_coup_valide(%p, 2, 3) =%d\n", (void*)j, jeu_est_coup_valide(j, 2, 3)); // DEBUG
	getchar();
	...
	while(jeu_est_partage(j) && nb_it <= 1000) {
		nb_it ++;
	printf("it%d: jeu_est_coup_valide(%p, 2, 3) =%d\n", nb_it, (void*)j, jeu_est_coup_valide(j, 2, 3)); // DEBUG
	getchar();
		...
		if(j->etat == JEU_ETAT_PLACEMENT) {
			...
			printf("DEBUG ********* Coup joué %d, %d (taper une touche pour poursuivre)\n", x, y);
			getchar();
		}
